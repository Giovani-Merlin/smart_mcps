# feat: greenfield cross-stack service

The Observatory shape in miniature (docs/orchestrators_improvements.md): two
hubs (a utility scaffold everything depends on, an aggregator verification
pass that depends on every slice), three two-task cross-stack vertical
slices joined only by matched route tags, and one isolated doc task.
