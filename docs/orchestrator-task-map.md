...
| `slice`       | no       | string \| null            | Vertical-slice label. Slice-mates are **contracted into one node** before Louvain — a hard must-link. Shared-infra / cross-cutting tasks carry **no** slice (they are hub material, never forced into a feature slice).         |
